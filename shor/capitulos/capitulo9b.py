...
def parte9b(cena):
    ...
    cena.play(Flash(caixas[0].get_center(), color=VERDE, flash_radius=0.55),
              run_time=0.6 * VEL)
    ...
    # a cadeia cumpriu o papel: sai de cena (r = 4 fica, sobe para o canto)
    ...
    cap5 = T("às vezes a ordem modular não dá informação útil",
             24).move_to([0, 2.45, 0])
    cena.play(FadeIn(cap5), run_time=0.8 * VEL)
    ...
    cena.play(ReplacementTransform(fim.copy(), r1), run_time=1.1 * VEL)
    cena.wait(0.6 * VEL)
