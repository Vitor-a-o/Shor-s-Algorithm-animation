...
def parte9(cena):
    ...
    cena.play(Create(fecha), Indicate(c00, color=VERDE), run_time=1.0 * VEL)
    ...
    # a tabela já cumpriu o papel: sai de cena para dar lugar à definição
    cena.play(FadeOut(VGroup(head_c, head_l, lin_h, lin_v, *linhas_cel,
                             seta4, zig)), run_time=0.8 * VEL)
    ...
    caixa_d.move_to([-2.9, -0.5, 0])
    borda = SurroundingRectangle(caixa_d, color=VERDE, buff=0.25,
                                 corner_radius=0.12)
    cena.play(Write(caixa_d), Create(borda), run_time=1.4 * VEL)
    cena.wait(1.8 * VEL)
