...
#   Narração: C6N01 … C6N13 (roteiro_video3_do_teorema_ao_rsa.md)
# ============================================================================
def parte6(cena):
    ...
    with narra(cena, "C6N01", 5.4):
        cena.play(FadeIn(titulo_tab, run_time=0.9 * VEL),
                  FadeIn(head_c, run_time=0.9 * VEL),
                  FadeIn(head_l, run_time=0.9 * VEL),
                  Create(lin_h, run_time=0.9 * VEL),
                  Create(lin_v, run_time=0.9 * VEL),
                  LaggedStart(*[FadeIn(l) for l in linhas_cel], lag_ratio=0.12,
                              run_time=1.8 * VEL))
    ...
    A = VGroup(*[T(str(j + 1), 30, PRETO).move_to([xsr[j], yA, 0])
                 for j in range(6)])
    with narra(cena, "C6N02", 6.7):
        cena.play(Create(el1, run_time=0.7 * VEL),
                  LaggedStart(*[TransformFromCopy(linhas_cel[0][j], A[j])
    ...
    with narra(cena, "C6N06", 8.3):
        cena.play(LaggedStart(*[Create(a) for a in arcos], lag_ratio=0.12),
                  run_time=1.8 * VEL)

    # produto da linha 3 escrito como (3·1)·(3·2)·…·(3·6)  (slide 61), a nota
    # de comutatividade e a condensação em 3⁶ (slide 62) — emendados: eq0
    # nasce, a nota aparece e, sem pausa, a virada em eq consome as duas
    ...
    with narra(cena, "C6N09", 8.8):
        cena.play(Write(div_e), Write(div_d), run_time=0.9 * VEL)
    ...
    # A VIRADA: a tabela inteira e o aparato de exemplo saem, e no mesmo
    ...
    saida = AnimationGroup(
        ...
        FadeOut(el1), FadeOut(el3), FadeOut(A), FadeOut(B),
        FadeOut(prods), FadeOut(mod_rot), FadeOut(arcos),
        geral.animate.move_to([0, -0.5, 0]).scale(40 / 28),
        Write(titulo), run_time=1.0 * VEL,
    )
    with narra(cena, "C6N12", 2.9):
        cena.play(LaggedStart(saida, FadeOut(resultado, run_time=0.6 * VEL),
                              lag_ratio=0.4))

    with narra(cena, "C6N13", 6.7):
        cena.play(Indicate(geral[4], color=LARANJA), run_time=1.0 * VEL)
